...
const _send = ({ subject, emails, message }, cb = null) => {
    const mailOptions = { 
        from: "no-response@lojabuilderseunegocioonline.com.br",
        to: emails,
        subject,
        html: message
    };
...
const enviarNovoPedido = ({ usuario, pedido }) => {
    const message = `
        ...
        <br /><br />
        <p>Atenciosamente,</p>
        <p>Equipe - Loja da Builder Seu Negócio Online</p>
    `;
    _send({
        subject: "Pedido Recebido - Loja da Builder Seu Negócio Online",
...
const cancelarPedido = ({ usuario, pedido }) => {
    ...
    `;
    _send({
        subject: "Pedido Cancelado - Loja da Builder Seu Negócio Online",
        emails: usuario.email,
        message
    });
}
...
const atualizarPedido = ({ usuario, pedido, status, data, tipo }) => {
    ...
    `;
    _send({
        subject: "Pedido Atualizado - Loja da Builder Seu Negócio Online",
        emails: usuario.email,
        message
    });
}
